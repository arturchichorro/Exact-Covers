...
const PlayableSudoku: React.FC<SudokuGridProps> = ({ initialGrid }) => {
  ...
  const isValidMove = useCallback((row: number, col: number, value: number): boolean => {
    for (let i = 0; i < 9; i++) {
  ...
  const handleKeyPress = useCallback((e: KeyboardEvent) => {
    ...
    if (num >= 0 && num <= 9) {
      ...
      if (num === 0) {
        ...
        setInvalidCells(prev => new Set([...prev].filter(cell => cell !== `${row}-${col}`)));
        return;
      }
      ...
      if (isValidMove(row, col, num)) {
        ...
        setInvalidCells(prev => new Set([...prev, `${row}-${col}`]));
      }
    }
  }, [selectedCell, isValidMove]);
        ...
        {grid.map((row, rowIndex) => (
          <div key={rowIndex} className="flex">
            {row.map((_, colIndex) => {
              ...
              const isValid = !invalidCells.has(`${rowIndex}-${colIndex}`);
